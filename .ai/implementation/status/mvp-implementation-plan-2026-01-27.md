...
**Data:** 2026-01-27  
**Ostatnia weryfikacja względem kodu:** 2026-01-29  
...
`.ai/implementation/progress/generations-study-nav-implementation-progress.md`
...
**Plan:** `.ai/implementation/endpoint/flashcards-crud-implementation-plan.md` (sekcja 2.1)
...
**Plan:** `.ai/implementation/endpoint/flashcards-crud-implementation-plan.md` (sekcja 2.2)
...
**Plan:** `.ai/implementation/endpoint/flashcards-crud-implementation-plan.md` (sekcja 2.3)
...
**Plik:** `src/pages/api/flashcards/[id].ts` (rozszerzenie)
**Zależności:** Zadanie 1.1.2
**Plan:** `.ai/implementation/endpoint/flashcards-crud-implementation-plan.md` (sekcja 2.4)
...
#### [x] Zadanie 1.2.1: Widok /flashcards - lista fiszek

**Plik:** `src/pages/flashcards.astro` (nowy)
**Zależności:** Zadanie 1.1.1
**Plan:** `.ai/implementation/endpoint/flashcards-crud-implementation-plan.md` (sekcja 3)
...
#### [x] Zadanie 1.2.2: Modal edycji fiszki

**Plik:** `src/components/FlashcardEditModal.tsx` (nowy)
**Zależności:** Zadanie 1.1.3, 1.2.1
**Plan:** `.ai/implementation/endpoint/flashcards-crud-implementation-plan.md` (sekcja 4)
...
#### [x] Zadanie 1.2.3: Usuwanie fiszki z potwierdzeniem

**Plik:** `src/components/FlashcardListView.tsx` (rozszerzenie)
**Zależności:** Zadanie 1.1.4, 1.2.1
**Plan:** `.ai/implementation/endpoint/flashcards-crud-implementation-plan.md` (sekcja 5)
...
#### [x] Zadanie 1.2.4: Ręczne dodawanie fiszki
...
**Zależności:** Brak
**Plan:** `.ai/implementation/endpoint/generations-get-endpoint-implementation-plan.md` (sekcja 2.1)
...
**Plan:** `.ai/implementation/endpoint/generations-get-endpoint-implementation-plan.md` (sekcja 2.2)
**Data zakończenia:** 2026-01-27
...
**Szczegółowy plan:** `.ai/implementation/view/generations-view-implementation-plan.md`
...
- `.ai/implementation/progress/generations-study-nav-implementation-progress.md` - Fazy 1-5 (Generations READ backend, Study Session, Navigation)
- `.ai/implementation/view/generations-view-implementation-plan.md` - Widok /generations (Zadanie 1.3.3)

**To jest KOMPLETNA DZIAŁAJĄCA APLIKACJA gotowa do testowania!**

**Następny krok:** ETAP 3 (SSR + refaktor, usunięcie DEFAULT_USER_ID)

---

## ETAP 2: Autentykacja - UI i Endpoints ✅ ZAKOŃCZONY

Cel: Dodać ekrany logowania/rejestracji (zrealizowany)

### 2.1. Auth Endpoints (Priorytet: KRYTYCZNY)

#### [x] Zadanie 2.1.1: POST /api/auth/register

**Plik:** `src/pages/api/auth/register.ts` (nowy)
**Zależności:** Brak
**Plan:** `.ai/implementation/endpoint/auth-endpoint-implementation-plan.md` (sekcja 2.1)
...
#### [x] Zadanie 2.1.2: POST /api/auth/login

**Plik:** `src/pages/api/auth/login.ts` (nowy)
**Zależności:** Brak
**Plan:** `.ai/implementation/endpoint/auth-endpoint-implementation-plan.md` (sekcja 2.2)
...
#### [x] Zadanie 2.1.3: POST /api/auth/logout

**Plik:** `src/pages/api/auth/logout.ts` (nowy)
**Zależności:** Brak
**Plan:** `.ai/implementation/endpoint/auth-endpoint-implementation-plan.md` (sekcja 2.3)
...
#### [x] Zadanie 2.2.1: Strona /login

**Plik:** `src/pages/login.astro` (nowy)
**Zależności:** Zadanie 2.1.2
**Plan:** `.ai/coreDocumentation/auth-spec.md` (sekcja 2.1.1)
...
#### [x] Zadanie 2.2.2: Strona /register
...
#### [x] Zadanie 2.2.3: Przycisk Logout w Navigation
...
## CHECKPOINT 2: Aplikacja z auth UI ✅ ZAKOŃCZONY

**Na tym etapie masz:**

- ✅ Wszystko z CHECKPOINT 1
- ✅ Endpointy auth: register, login, logout
- ✅ Strony /login, /register z formularzami (LoginForm, RegisterForm)
- ✅ UserMenu w Navigation (logout) + przyciski Login/Register gdy niezalogowany
- ⚠️ **ALE:** Nadal używa `DEFAULT_USER_ID` w endpointach (lub locals.user – do weryfikacji)
- ⚠️ **ALE:** Middleware może nie wymuszać sesji na wszystkich ścieżkach

**To przygotowanie pod ETAP 3 (SSR + pełny auth).**
...
| **ETAP 2**   | 6/6 zadań       | ✅ ZAKOŃCZONY                      | 🔴 KRYTYCZNY    |
| **ETAP 3**   | 12 zadań        | ⏳ DO ZROBIENIA                    | 🔴 KRYTYCZNY    |
| **ETAP 4**   | 6 zadań         | 🟢 OPCJONALNY                      | 🟢 NICE TO HAVE |
| **SUMA MVP** | **20/32 zadań** | **62% ZAKOŃCZONE** (ETAP 1+2 done) | -               |

**Postęp:**

- ✅ ETAP 1: 14/14 zadań (100%) - **WSZYSTKIE zadania włącznie z opcjonalnym 1.3.3**
- ✅ ETAP 2: 6/6 zadań (100%) - **auth endpoints + strony /login, /register + UserMenu/Logout**
- ⏳ ETAP 3: 0/12 zadań (0%)
...
5. **Zgodność z planami** - trzymaj się `.ai/implementation/**/*.md` i `.ai/coreDocumentation/*.md`
...
1. ✅ ETAP 1 - ZAKOŃCZONY
2. ✅ ETAP 2 - ZAKOŃCZONY (auth UI + endpoints)
3. ⏳ **ETAP 3** - SSR + refaktor (prawdziwy auth, usunięcie DEFAULT_USER_ID) ← **NASTĘPNY KROK**
4. 🎉 PRODUKCJA
...
2. **ETAP 2:** ✅ Zakończony (login/register, auth endpoints, UserMenu)
3. **ETAP 3:** SSR + refaktor (prawdziwy auth, usunięcie DEFAULT_USER_ID) ← **następny**
...
| 2026-01-29 | Aktualizacja planu: ścieżki do planów (.ai/implementation, .ai/coreDocumentation), spójność checkboxów 1.2.x, sekcja Review                                                                                                         |
| 2026-01-29 | **ETAP 2 ZAKOŃCZONY** – odhaczenie zadań 2.1.1–2.1.3, 2.2.1–2.2.3; CHECKPOINT 2 i tabela podsumowania zaktualizowane                                                                                                                |
| 2026-01-29 | Weryfikacja względem kodu: dodano „Ostatnia weryfikacja”, poprawka linku do planu aktualizacji (ToDo → .ai/implementation/plan), adnotacja o nazwach w kodzie (EditFlashcardModal, FlashcardsView; getFlashcards, getFlashcardById) |

---

## Review (aktualizacja 2026-01-29)

**Cel:** Dopasowanie `mvp-implementation-plan-2026-01-27.md` do aktualnej struktury repo i stanu zadań.

**Wprowadzone zmiany:**

1. **Checkboxy** – W sekcjach szczegółowych zadania 1.2.1, 1.2.2, 1.2.3, 1.2.4 miały status ✅ w nagłówku ETAP 1, ale w treści nadal `[ ]`. Zmieniono na `[x]` (1.4.1 był już oznaczony jako wykonany).

2. **Ścieżki do planów** – Pliki z `ToDo/` zostały przeniesione do `.ai/implementation/` (git: D). Zaktualizowano odnośniki:
   - `ToDo/generations-study-nav-implementation-progress.md` → `.ai/implementation/progress/generations-study-nav-implementation-progress.md`
   - `ToDo/generations-view-implementation-plan.md` → `.ai/implementation/view/generations-view-implementation-plan.md`
   - `.ai/flashcards-endpoint-implementation-plan.md` → `.ai/implementation/endpoint/flashcards-crud-implementation-plan.md`
   - `.ai/generations-get-endpoint-implementation-plan.md` → `.ai/implementation/endpoint/generations-get-endpoint-implementation-plan.md`
   - `.ai/auth-endpoint-implementation-plan.md` → `.ai/implementation/endpoint/auth-endpoint-implementation-plan.md`
   - `.ai/auth-spec.md` → `.ai/coreDocumentation/auth-spec.md`
   - W sekcji „Zasady wykonywania zadań” ustawiono odwołanie do `.ai/implementation/**/*.md` i `.ai/coreDocumentation/*.md`.

3. **Changelog** – Dodano wpis z datą 2026-01-29.

**ETAP 2:** Zweryfikowano – zadania 2.1.1–2.1.3 (auth endpoints) i 2.2.1–2.2.3 (strony /login, /register, UserMenu/Logout) są zaimplementowane. Status ETAP 2 zaktualizowany na ZAKOŃCZONY.

**Zgodność nazw z kodem (2026-01-29):** W repozytorium: komponenty `EditFlashcardModal.tsx`, `FlashcardsView.tsx` (plan używa nazw FlashcardEditModal, FlashcardListView); FlashcardService: `getFlashcards()`, `getFlashcardById()` (plan: getAll, getById). Plan pozostaje historyczny – nazwy w tekście nie zmieniane.

**Plan aktualizacji plików statusu:** [status-files-update-plan-2026-01-29.md](../plan/status-files-update-plan-2026-01-29.md)
